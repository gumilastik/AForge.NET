...
namespace VVVV.AForge.Video.DirectShow
{
    ...
    using VVVV.AForge.Video.DirectShow.Internals;
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        public event UpdateBufferEventHandler UpdateBuffer;
        ...
        private void SetResolution(IAMStreamConfig streamConfig, VideoCapabilities resolution)
        {
            ...
            for (int i = 0; i < capabilitiesCount; i++)
            {
                try
                {
                    ...
                    if (resolution == vc)
                    {
                        ...
                        {
                            framerate = Math.Min(Math.Max(framerate, vc.MinimumFrameRate), vc.MaximumFrameRate);
                            break;
                        }
                    }
                }
            ...
            {
                if (newMediaType.FormatType == FormatType.VideoInfo)
                {
                    VideoInfoHeader videoInfo = (VideoInfoHeader)Marshal.PtrToStructure(newMediaType.FormatPtr, typeof(VideoInfoHeader));
                    videoInfo.AverageTimePerFrame = (int)(10000000 / framerate);
                    Marshal.StructureToPtr(videoInfo, newMediaType.FormatPtr, false);
                }
                else if (newMediaType.FormatType == FormatType.VideoInfo2)
                {
                    VideoInfoHeader2 videoInfo = (VideoInfoHeader2)Marshal.PtrToStructure(newMediaType.FormatPtr, typeof(VideoInfoHeader2));
                    videoInfo.AverageTimePerFrame = (int)(10000000 / framerate);
                    Marshal.StructureToPtr(videoInfo, newMediaType.FormatPtr, false);
                }
        ...
        private void OnUpdateBuffer(IntPtr buffer, int bufferLen, int width, int height)
        {
            framesReceived++;
            bytesReceived += bufferLen;

            if ((!stopEvent.WaitOne(0, false)) && (UpdateBuffer != null))
                UpdateBuffer(this, new UpdateBufferEventArgs(buffer, bufferLen, width, height));
        }
                ...
                if (parent.UpdateBuffer != null)
                {
                    unsafe
                    {
                        ...
                        for (int i = 0; i < height; ++i)
                        {
                            ...
                            {
                                int n = (i * width + j) * 4;
                                int m = (i * width + (width - j - 1)) * 4;
                                
                                /*
                                 byte swap = buffer[n];

                                buffer[n*4].a = buffer[m].a;
                                buffer[n].r = buffer[m].b;
                                buffer[n].g = buffer[m].g;
                                buffer[n].b = buffer[m].r;
                                 */
                            }
                        }
                    ...
                    parent.OnUpdateBuffer(buffer, bufferLen, width, height);
                }
               ...
               if (parent.NewFrame != null)
                {
                    ...
                    // create new image
                    System.Drawing.Bitmap image = new Bitmap( width, height, PixelFormat.Format32bppArgb);
